...
def send_mail(subject, files=[]):
    ...
    EMAIL_HOST_USER = 'AKIAJNNVXWT5BGVV6B2A'
    EMAIL_HOST_PASSWORD = 'AnZV5CSTNR7xsa+Tp9SRaMFYzaeqeLj8xHRRuFheajRu'
    EMAIL_PORT = 587

    send_from = "petekawuki@gmail.com"
    send_to = "petekawuki@yahoo.com"
    text = "Here are the latest reports..."
    msg = MIMEMultipart()
    msg['From'] = send_from
    msg['To'] = send_to
...
def main():
    ...
    subject =  hostname + ": System run at: " + time_string
    ...
    if not os.path.isfile(dir_filename):
        ...
        f_contracts = "/results/contracts.csv"
        contracts_df.to_csv(f_contracts)
        ...
        f_positions = "/results/positions.csv"
        with_broker_positions_df.to_csv(f_positions)

        trades_df = generate_reports(with_broker_positions_df)

        f_trades = "/results/trades.csv"
        trades_df.to_csv(f_trades) #/pysystemtrade/private/SystemR

        fileArray = [f_contracts, f_positions, f_trades]
        send_mail( subject , files=fileArray)
